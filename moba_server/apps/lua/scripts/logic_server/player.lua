...
function player:init(uid, s, ret_handler)
	...
	self.seatid=-1--玩家座位id
	self.sidee=1--玩家在游戏里面所在的边,0(lhs),1{rhs}
	self.heroid=-1--玩家英雄号
	self.state = State.InView -- 玩家当前处于旁观状态
	self.is_robot = false -- 玩家是否为机器人
	self.client_ip=nil --玩家对应客户端的upd的ip地址
	self.client_udp_port=0 --玩家对应的客户端的udp的port
	-- 数据库理面读取玩家的基本信息;
...
function player:udp_send_cmd(stype, ctype, body) 
	...
	print(self.client_ip, self.client_udp_port, msg[1],msg[2],msg[4])
